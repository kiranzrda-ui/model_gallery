...
def load_sanitized_data():
    ...
    if 'approval_status' in df.columns:
        df['approval_status'] = df['approval_status'].str.lower().replace({'pending_review': 'Pending', 'approved': 'Approved'})
    return df
...
def call_gemini(prompt, api_key):
    ...
    except: return "Assistant: I am currently operating in limited offline mode."

def hybrid_search(query, df):
    if not query or df.empty: return df
    q = query.lower()
    # Filter Official
    if "official" in q: df = df[df['type'].str.lower() == 'official']
    # Textual
...
def render_tile(row, user, prefix, result_count, is_companion=True):
    ...
    if result_count > 1:
        with c1:
            if st.button("Compare", key=f"c_{prefix}_{row['name']}"):
                ...
                else: st.session_state.basket.append(row['name'])
                st.rerun()
    with c2:
...
def run_companion():
    ...
    for i, msg in enumerate(st.session_state.messages):
        with st.chat_message(msg["role"]):
            ...
            if "bar_data" in msg:
                bdf = pd.DataFrame(msg["bar_data"])
                if not bdf.empty: st.plotly_chart(px.bar(bdf, x='name', y=['accuracy','latency','usage'], barmode='group'))
            if "pie_data" in msg:
                pdf = pd.DataFrame(msg["pie_data"])
                if not pdf.empty: st.plotly_chart(px.pie(pdf, values='revenue_impact', names='domain', hole=0.4))
    ...
    if prompt := st.chat_input("Ask a question..."):
        ...
        with st.chat_message("assistant"):
            ...
            if "submit" in q or "register" in q or st.session_state.context["intent"] == "SUB":
                ...
                raw = call_gemini(f"Extract JSON (name, use_cases, domain, accuracy, latency, usage, revenue_impact). Text: {prompt}", api_key)
                try:
            ...
            # NAT PATEL APPROVALS
            elif user_role == "Nat Patel" and ("approve" in q or "queue" in q):
                ...
                if "approve" in q:
                    ...
                    if m_name:
                        ...
                        df_master.to_csv(REG_PATH, index=False); res_txt = f"✅ Approved **{m_name}**."
                    else: res_txt = f"Found {len(pend)} items. Specify a name to approve."
                else:
                    res_txt = f"You have {len(pend)} pending approvals."
                    res_df = pend
            ...
            # ROI
                ...
                res_txt = "Revenue impact contribution by domain:"
            ...
            # COMPARE
                ...
                else: res_txt = "Provide exact names to compare."
            ...
            st.session_state.messages.append({"role": "assistant", "content": res_txt, "df_data": res_df.to_dict('records'), "pie_data": res_pie, "bar_data": res_bar})
...
if not app_mode:
    ...
    with t[-1]: # Dynamic tail tab
        if user_role == "Nat Patel":
            st.header("Leader Approval Gateway")
            pend = df_master[df_master['approval_status'] == 'Pending']
            st.dataframe(pend[['name','contributor','domain','accuracy']])
